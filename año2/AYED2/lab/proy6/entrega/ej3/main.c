...
bool is_valid_option(char option)
{
  ...
  result = (option == ADD || option == REMOVE || option == DUMP || option == EMPTY || option == LOAD || option == SEARCH || option == SHOW || option == SIZE || option == QUIT);
...
{
  string word = get_input("Please enter the word to delete from the dict");
  if (!dict_exists(current, word)) {
    printf(RESULT_PREFIX "The word does not exist in the dict.\n");
  } else {
