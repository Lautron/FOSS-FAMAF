...
int main(int argc, char* argv[])
{
  ...
  int min_t = get_min_historic_temp(array);
  printf("\nMinTemp: %d\n", min_t);

  int res[YEARS];
  get_max_temp_per_year(array, res);
  printf("MaxTempPerYear:\n");
  imprimeArreglo(res, YEARS);

  int res2[YEARS];
  get_max_prec_month(array, res2);
  printf("MaxPrecMonthPerYear:\n");
  imprimeArreglo(res2, YEARS);
